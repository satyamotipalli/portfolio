...
export default function NetworkGraphic() {
  return (
    <div className="relative aspect-square w-full max-w-[440px]">
      ...
      <div className="absolute left-1/2 top-1/2 h-[28%] w-[28%] -translate-x-1/2 -translate-y-1/2 rounded-full bg-accent/16 blur-[32px]" />

      <svg viewBox="0 0 400 400" className="relative h-full w-full">
        <defs>
          <radialGradient id="ng-core" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#ffffff" stopOpacity="1" />
            <stop offset="16%" stopColor="var(--accent)" stopOpacity="0.72" />
            <stop offset="42%" stopColor="var(--accent)" stopOpacity="0.16" />
            <stop offset="100%" stopColor="var(--accent)" stopOpacity="0" />
          </radialGradient>
          <linearGradient id="ng-edge" gradientUnits="userSpaceOnUse" x1="60" y1="60" x2="340" y2="340">
        ...
        {/* spokes core -> vertices (all six, including the vertical top/bottom) */}
        {v.map((p, i) => (
        ...
        {/* travelling particles: flow from each node inward to the core */}
        {travellers.map((t, i) => (
          <motion.circle
            key={`t-${i}`}
            r="3"
        ...
        {/* glowing core */}
        <circle cx="200" cy="200" r="62" fill="url(#ng-core)" />
        <motion.circle
          cx="200"
          cy="200"
          r="22"
